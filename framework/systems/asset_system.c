...
static struct Asset_System {
	...
	struct Hash_Table_U32 reflex; // name string id : `struct Handle`
...
void asset_system_init(void) {
	gs_asset_system = (struct Asset_System){
		...
		.reflex = hash_table_u32_init(sizeof(struct Handle)),
...
void asset_system_free(void) {
	FOR_HASH_TABLE_U32 (&gs_asset_system.types, it_type) {
	...
	hash_table_u32_free(&gs_asset_system.reflex);
...
void asset_system_del_type(struct CString type_name) {
	...
	struct Asset_Type * type = hash_table_u32_get(&gs_asset_system.types, type_id);
	if (type == NULL) { return; }

	FOR_HANDLE_TABLE (&type->instances, it_inst) {
		struct Asset_Inst * inst = it_inst.value;

		hash_table_u32_del(&gs_asset_system.reflex, inst->header.name_id);
...
struct Handle asset_system_aquire(struct CString name) {
	...
	struct Handle const * meta_handle_ptr = hash_table_u32_get(&gs_asset_system.reflex, name_id);
	...
	hash_table_u32_set(&gs_asset_system.reflex, name_id, &meta_handle);
...
void asset_system_discard(struct Handle handle) {
	...
	hash_table_u32_del(&gs_asset_system.reflex, meta->name_id);
	handle_table_discard(&gs_asset_system.meta, handle);

	struct Asset_Type * type = hash_table_u32_get(&gs_asset_system.types, meta->type_id);
	if (type == NULL) { return; }

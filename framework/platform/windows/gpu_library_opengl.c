...
#define GET_VALUE(key) dictionary_int_int_get_value(request_keys, request_vals, key)
	...
	int formats_count = 0;
	struct Pixel_Format * formats = ALLOCATE_ARRAY(struct Pixel_Format, formats_capacity + 1);
...
static struct Pixel_Format * allocate_pixel_formats_legacy(HDC device) {
	...
	int formats_count = 0;
	struct Pixel_Format * formats = ALLOCATE_ARRAY(struct Pixel_Format, pfd_count + 1);

...
static uint32_t hash_set_u32_find_key_index(struct Hash_Set_U32 * hash_set, uint32_t key_hash);
void hash_set_u32_resize(struct Hash_Set_U32 * hash_set, uint32_t target_capacity) {
	...
	uint32_t * key_hashes = hash_set->key_hashes;
	...
	for (uint32_t i = 0; i < capacity; i++) {
		...
		uint32_t const key_index = hash_set_u32_find_key_index(hash_set, key_hashes[i]);
...
bool hash_set_u32_get(struct Hash_Set_U32 * hash_set, uint32_t key_hash) {
	...
	uint32_t const key_index = hash_set_u32_find_key_index(hash_set, key_hash);
...
bool hash_set_u32_set(struct Hash_Set_U32 * hash_set, uint32_t key_hash) {
	if (should_hash_table_grow(hash_set->capacity, hash_set->count)) {
		...
		hash_set_u32_resize(hash_set, target_capacity);
	}

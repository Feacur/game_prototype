...
static void buffer_ensure_capacity(struct Buffer * buffer, uint64_t target_count) {
	if (buffer->capacity < target_count) {
		uint64_t const target_capacity = grow_capacity_value_u64(buffer->capacity, target_count - buffer->capacity);
...
void buffer_align(struct Buffer * buffer) {
	uint64_t const target_count = align_u64(buffer->count);

...
struct Hash_Table_U64 * hash_table_u64_init(uint32_t value_size) {
	...
	struct Hash_Table_U64 * hash_table = MEMORY_ALLOCATE(struct Hash_Table_U64);
...
bool hash_table_u64_set(struct Hash_Table_U64 * hash_table, uint64_t key_hash, void const * value) {
	...
	uint32_t const key_index = hash_table_u64_find_key_index(hash_table, key_hash);
	bool const is_new = hash_table->marks[key_index] != HASH_TABLE_U64_MARK_FULL;
...
static uint32_t hash_table_u64_find_key_index(struct Hash_Table_U64 * hash_table, uint64_t key_hash) {
	...
	for (uint32_t i = 0; i < hash_table->capacity; i++) {
		...
		if (mark == HASH_TABLE_U64_MARK_SKIP) { continue; }
		if (mark == HASH_TABLE_U64_MARK_NONE) { return index; }
